...
def mapfn1(k, points):
    ...
    Mapper просто пробегает по точкам шарда и проверяет, может ли текущая точка стать центром нового зонтика (т.е.
        проверка на то, что точка не входит ни в один Т2 радиус уже имеющихся центров). Если может, то добавляем её в
        список центров.
    ...
    Еще замечание: промежуточные (и не только) результаты записываются на DFS,
        но у нас игрушечный пример, поэтому все происходит в RAM.
    """
...
def mapfn2(k, points):
    ...
    Mapper делает довольно очевидную вещь: для каждой точки вычисляет дистанции между ней
        и центрами зонтиков, и приписывает зонтик к точке, если дистанция не больше T1.
    """
    dist = lambda x, y: sum(map(lambda x_i, y_i: (x_i - y_i) ** 2, x, y))
...
def mapfn3(k, items):
    ...
    Вместе с точками на вход mapper получает центроиды. И центроиды, и точки помечены соответствующими им центрами
        зонтиков. Для того, чтобы считать расстояние от точки до центроиды, нужно убедиться, что у точки есть общий
        зонтик хотя бы с одним из центроидов.
    """
    dist = lambda x, y: sum(map(lambda x_i, y_i: (x_i - y_i) ** 2, x, y))
