...
# соотносит каждому ключу имя шарда, в котором он содержится 
# и порядковый номер фрагмента этого шарда.
def find_keys(keys, partitions):
  ...
  while key_num < len(keys):
    ...
    if (key_num < len(keys)):
      # читаем следующий только если для данного диапазона больше нет ключей
...
# принимает словарь, полученный в find_keys и выдаёт собственно имена фрагментов
...
# принимает массив из ключей, итератор содержимого файла и счётчик, сколько раз какой ключ встретился
...
def calculate_sum(keys_filename):
  ...
  # получим диапазоны фрагментов
  partitions = get_file_content("partitions")
  # получим названия фрагментов
  fragments = get_fragment_name(find_keys(keys, partitions))
  shards = defaultdict(list)
  keys_count = Counter(keys) # не будем терять повторяющиеся ключи
  for key in fragments: # раньше каждому ключу сопостовляли фрагмент. Теперь имеющимся фрагментом сопоставим ключи, в них лежащие
    shards[fragments[key]].append(key)
  total_sum = 0
  for shard in shards:
    total_sum += get_sum_in_file(shards[shard], get_file_content(shard), keys_count)
  return total_sum
